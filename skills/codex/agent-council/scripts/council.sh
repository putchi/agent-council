...
if ! command -v node >/dev/null 2>&1; then
  ...
  echo "Codex skills cannot bundle or auto-install Node." >&2
  echo "" >&2
...
# Host agents cannot update native TODO/plan UIs while a long-running
# command is executing. If we're in a host agent context, return immediately with a single `wait`
# JSON payload (includes `.ui.codex.update_plan.plan`) and let the
